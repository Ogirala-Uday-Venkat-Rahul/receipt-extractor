...
The frontend loads instantly, but the model API behind it is on HF's free CPU tier, so it
sleeps after inactivity and cold-starts on the next call (re-downloads the GGUF and warms
llama.cpp), and even warm it takes **about 2-3 minutes per receipt**. So don't present a live
extraction cold.
...
3. If you want to show a *live* run, kick one off about 3 minutes before you need it to warm
...
- The total was labelled **AMOUNT DUE**, not "TOTAL", but it found it anyway.
...
- **The dataset is synthetic.** It models the kinds of noise real receipts have, but
  ...
  date, and total stay correct, but it sometimes *over-applies* the quantity-times-unit-price
...
- **CPU serving is slow.** About 2-3 minutes per receipt on the free Space, a 7B on 2 vCPUs.
